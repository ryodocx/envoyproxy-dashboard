...
type RouteFunc func(context.Context, *db.RouteMutation) (db.Value, error)

// Mutate calls f(ctx, m).
func (f RouteFunc) Mutate(ctx context.Context, m db.Mutation) (db.Value, error) {
	mv, ok := m.(*db.RouteMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *db.RouteMutation", m)
	}
	return f(ctx, mv)
}
